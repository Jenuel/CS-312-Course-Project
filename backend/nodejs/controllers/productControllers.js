...
const getProducts = async (request, response) => {
  ...
  try {
    let query =
      'SELECT p.ProductID AS "ProductID", p.name AS "Name", p.StocksRemaining AS "Stocks" , p.Price AS "Price", p.status AS "Status", p.Image AS "Image" FROM `product` p  WHERE p.BoothID = ? ';
    let params = [boothId];
