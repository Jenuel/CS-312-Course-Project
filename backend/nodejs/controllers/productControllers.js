  ...
  try {
    ...
    if (orders[0].orderCount > 0) {
      return response.status(400).json({
        message:
          "Cannot delete product that has associated orders. Consider deactivating it instead.",
      });
    }
    ...
    try {
      ...
      await db.query("DELETE FROM inventory WHERE ProductID = ?", [productId]);
...
const updateStockInInventory = async (value, productId, db) => {
  try {
    ...
    const { "current stocks": currentStocks, "updated stocks": updatedStocks } =
      difference[0];
