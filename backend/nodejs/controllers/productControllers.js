  ...
  try {
    ...
    if (allPositive.length && allPositive[0].AllPositive) {
      ...
      await db.query(
        ...
        SELECT NULL, 
               p.ProductID, 
               COALESCE(DATE(o.DatePaid), CURRENT_DATE) AS DatePaid, 
               'out', 
               p.Quantity
        FROM \`order\` o
      ...
      const [breakdown] = await db.query(
        ...
        SELECT o.OrderID AS 'order id', 
               x.name AS 'product name', 
               x.Price AS 'product price', 
               p.Quantity AS 'number of ordered product', 
               p.Total AS 'total price per product', 
               o.Price AS 'overall total' 
        FROM \`order\` o 
        JOIN order_products p ON o.OrderID = p.OrderID 
